...
class DecoderLayer(Module):
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att):
        # MHA+AddNorm
        self_att, atts = self.self_att(input, input, input, mask_self_att)
        self_att = self.lnorm1(input + self.dropout1(self_att))
        self_att = self_att * mask_pad
        # MHA+AddNorm
        enc_att, atts = self.enc_att(self_att, enc_output, enc_output, mask_enc_att)
...
class DifnetDecoder(Module):
    ...
    def forward(self, input, encoder_output, mask_encoder):
        ...
        out = self.fc(out)
        merged_attn = att.mean(dim=1)  # 形状变为 (1, 1, 49)

        return F.log_softmax(out, dim=-1),merged_attn
