...
from ..captioning_model2 import CaptioningModel2
...
class Difnet(CaptioningModel2):
    def __init__(self, bos_idx, encoder, decoder):
        ...
        self.embed_region = nn.Sequential(
                nn.Linear(2048, 512),
                nn.ReLU(),
                nn.Dropout(p=0.1),
                nn.LayerNorm(512))
    ...
    def forward(self, images, seq, pixel, region, *args):
        images = self.embed_image(images)
        pixel = self.embed_pixel(pixel)
        detect = self.embed_region(region)
        enc_output, mask_enc, contrastive_loss = self.encoder(images, pixel, detect)
        dec_output = self.decoder(seq, enc_output, mask_enc)
        return dec_output, contrastive_loss

    def init_state(self, b_s, device):
        return [torch.zeros((b_s, 0), dtype=torch.long, device=device),
                None, None]

    def step(self, t, prev_output, visual, pixel, seq, region, mode='teacher_forcing', **kwargs):
        it = None
        if mode == 'teacher_forcing':
            ...
            if t == 0:
                ...
                pixel = self.embed_pixel(pixel)
                detect = self.embed_region(region)
                self.enc_output, self.mask_enc, _ = self.encoder(visual, pixel, detect)
    ...
    def step2(self, t, prev_output, visual, pixel, seq, region, mode='teacher_forcing', **kwargs):
        it = None
        if mode == 'teacher_forcing':
            raise NotImplementedError
        elif mode == 'feedback':
        ...
        return self.decoder(it, self.enc_output, self.mask_enc)
class TransformerEnsemble(CaptioningModel2):
