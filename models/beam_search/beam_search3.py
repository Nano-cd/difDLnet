...
class BeamSearch3(object):
    '''add segmentation feature and region feature'''
    ...
    def apply(self, visual: utils.TensorOrSequence, pixels: utils.TensorOrSequence,
              region: utils.TensorOrSequence, out_size=1, return_probs=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                visual, outputs, att = self.iter(t, visual, pixels, region, outputs, return_probs, **kwargs)
                atts.append(att)
        # Sort result
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence, pixels: utils.TensorOrSequence,
             region: utils.TensorOrSequence, outputs, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob, att_mask = self.model.step2(t, self.selected_words, visual, pixels,  None, region, mode='feedback', **kwargs)
        ...
        return visual, outputs, att_mask
