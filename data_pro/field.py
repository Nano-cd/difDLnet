...
class ImageDetectionsField152(RawField):
    ...
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
        ...
        super(ImageDetectionsField152, self).__init__(preprocessing, postprocessing)
...
class ImageAllFieldWithMask(RawField):
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        return precomp_data.astype(np.float32), relative_boxes.astype(np.float32), grid_data.astype(
...
class PixelField(RawField):
    '''return segmentation feature'''

    def __init__(self, preprocessing=None, postprocessing=None, pixel_path=None, max_detections=100,
                 sort_by_prob=False, load_in_tmp=True):
        self.max_detections = max_detections
        self.pixel_path = pixel_path
        self.sort_by_prob = sort_by_prob

        super(PixelField, self).__init__(preprocessing, postprocessing)

    def preprocess(self, x, avoid_precomp=False):
        image_id = int(x.split('_')[-1].split('.')[0])
        if 'train' in x:
            d_path = os.path.join(self.pixel_path, 'train2014/' + str(image_id) + '.npy')
        if 'val' in x:
            d_path = os.path.join(self.pixel_path, 'val2014/' + str(image_id) + '.npy')

        try:
            precomp_data = np.load(d_path)
        except KeyError:
            warnings.warn('Could not find Pixel for %d' % image_id)
            precomp_data = np.random.rand(133, 7, 7).astype(np.float32)
...
class ImageDetectionsField2(RawField):
    '''return image_id, grid feature'''
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
                 sort_by_prob=False, load_in_tmp=True):
        self.max_detections = max_detections
        ...
        super(ImageDetectionsField2, self).__init__(preprocessing, postprocessing)
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        return image_opened, precomp_data.astype(np.float32)
