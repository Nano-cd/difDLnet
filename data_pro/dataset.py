...
class PairedDataset(Dataset):
    def __init__(self, examples, fields):
        ...
        self.detect_field = self.fields['detect']  # 添加detect_field
    ...
    def image_dictionary(self, fields=None):
        if not fields:
            fields = self.fields
        dataset = DictionaryDataset(self.examples, fields, key_fields=['image', 'pixel', 'detect'])
        return dataset
...
class COCO(PairedDataset):
    def __init__(self, image_field, text_field, pixel_field, detect_field, img_root, ann_root, id_root=None, use_restval=True,
        ...
        for split in ['train', 'val', 'test']:
            ...
            for index in range(len(ids)):
                ...
                example = Example.fromdict({'image': os.path.join(img_root, filename),
                                            ...
                                            'detect': os.path.join(img_root, filename)})
