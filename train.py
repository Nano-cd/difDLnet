...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='difnet', choices=['difnet', 'difnet_PRO'])
    ...
    # Create the dataset
    dataset = COCO(image_field, text_field, pixel_field, 'dataset/coco/images', args.annotation_folder, args.annotation_folder)
    ...
    if args.mode == 'difnet_PRO':
        encoder = DifnetEncoder(1, 3, 3, 0, attention_module=ScaledDotProductAttention)
        decoder = DifnetDecoder(len(text_field.vocab), 54, 3, text_field.vocab.stoi['<pad>'])
        model = Difnet(text_field.vocab.stoi['<bos>'], encoder, decoder).to(device)

    dict_dataset_train = train_dataset.image_dictionary(
        {'image': image_field, 'text': RawField(), 'pixel': pixel_field})
