...
from models.diffnet_pro.decoders import DifnetDecoder
...
def train_scst(model, dataloader, optim, cider, text_field):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        for it, ((detections, pixels, region), caps_gt) in enumerate(dataloader):
            ...
            gt_keys = caps_gt.keys()
            res_keys = caps_gen.keys()

            if not (gt_keys == res_keys):
                continue  # 跳过越界索引
...
def train_greedy(model, dataloader, optim, cider, text_field):
    # Training with self-critical
    # tokenizer_pool = multiprocessing.Pool()
    running_reward = .0
    running_reward_baseline = .0
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        for it, ((detections, pixels, region), caps_gt) in enumerate(dataloader):
            ...
            # 生成贪婪解码结果（基线）
            with torch.no_grad():
                greedy_outs, _ = model.beam_search(
                    detections, pixels, region, seq_len,
                    text_field.vocab.stoi['<eos>'], beam_size=1, out_size=1
                )
            ...
            # Rewards
            caps_gen = text_field.decode(outs.view(-1, seq_len))
            caps_greedy = text_field.decode(greedy_outs.view(-1, seq_len))
            # 计算贪婪解码的奖励
            caps_gt_greedy = caps_gt
            caps_greedy = evaluation.PTBTokenizer.tokenize(caps_greedy)
            caps_gt_greedy = evaluation.PTBTokenizer.tokenize(caps_gt_greedy)

            gt_keys = caps_greedy.keys()
            res_keys = caps_gt_greedy.keys()

            if not (gt_keys == res_keys):
                continue  # 跳过越界索引
            reward_greedy = cider.compute_score(caps_gt_greedy, caps_greedy)[1].astype(np.float32)
            reward_greedy = torch.from_numpy(reward_greedy).to(device)

            # 计算Beam Search的奖励
            caps_gt_beam = list(itertools.chain(*([c, ] * beam_size for c in caps_gt)))
            caps_gen = evaluation.PTBTokenizer.tokenize(caps_gen)
            caps_gt_beam = evaluation.PTBTokenizer.tokenize(caps_gt_beam)
            gt_keys = caps_gt_beam.keys()
            res_keys = caps_gen.keys()

            if not (gt_keys == res_keys):
                continue  # 跳过越界索引
            reward_beam = cider.compute_score(caps_gt_beam, caps_gen)[1].astype(np.float32)
            reward_beam = torch.from_numpy(reward_beam).to(device).view(-1, beam_size)

            # 计算优势函数
            reward_baseline = reward_greedy.unsqueeze(-1)
            advantage = reward_beam - reward_baseline

            # 计算损失
            loss = -torch.mean(log_probs, -1) * advantage.detach()
            loss = loss.mean()
            ...
            # 更新进度条
            running_loss += loss.item()
            running_reward += reward_beam.mean().item()
            running_reward_baseline += reward_baseline.mean().item()
            pbar.set_postfix(loss=running_loss / (it + 1), reward=running_reward / (it + 1),
                             reward_baseline=running_reward_baseline / (it + 1))
            pbar.update()
...
if __name__ == '__main__':
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if not use_rl:
            ...
            optim = Adam(model.parameters(), lr=5e-6*0.5)
            # train_loss, reward, reward_baseline = train_greedy(model, dict_dataloader_train, optim, cider_train, text_field)
